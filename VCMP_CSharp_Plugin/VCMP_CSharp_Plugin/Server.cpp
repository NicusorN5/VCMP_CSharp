...
void VCMPCSharpPlugin::Server::CreateExplosion(int world, int type, Vector ^ pos, CPlayer ^ guilt_player, bool onground)
{
	int id;
...
void VCMPCSharpPlugin::Server::PlaySound(int world, int sound, Vector ^ pos)
{
	api->PlaySound(world, sound, pos->x, pos->y, pos->z);
}

void VCMPCSharpPlugin::Server::ShowMapObject(int model, Vector ^ pos)
{
	api->ShowMapObject(model, pos->x, pos->y, pos->z);
}

void VCMPCSharpPlugin::Server::HideMapObject(int model, Vector ^ pos)
{
